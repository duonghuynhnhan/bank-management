...
class Admin_Change_Password(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.label_new_password = tk.Label(self, text = "Input new password (10 digits)", font = (app_font, 12))
        ...
        self.button_show_password_again = tk.Button(self, text = "SHOW", font = (app_font, 8, "bold"), width = 5, border = "0", command = self.show_password_again)
        self.button_show_password_again.place(x = 325, y = 375)
    ...
    def confirm(self):
        ...
        if oldpassword and newpassword and newpasswordagain:
            code = """SELECT PASS FROM ADMIN_ACCOUNT
                    WHERE ACC_NUM = '%s'"""%(self.adminname)
            ...
            if oldpassword == db_password:
                if newpassword == newpasswordagain:
                    if len(newpassword) == 10:
                        code = "CALL CHANGE_ADMIN_PASSWORD('%s', '%s')"%(self.adminname, newpassword)
                        self.controller.cursor.execute(code)
                        done = messagebox.showinfo(title = "Change password", message = "You have changed password successfully!")
                        if done:
                            self.delete()
                            self.controller.show_frame("Admin_Account_Information")
                    else:
                        messagebox.showerror(title = "Change password", message = "New password must has 10 digits!")
                        self.delete()
                else:
