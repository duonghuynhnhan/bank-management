...
class User_Forgot_Password(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.label_username = tk.Label(self, text = "Input username", font = (app_font, 12))
        self.label_username.pack(pady = 40)
        self.entry_username = tk.Entry(self, font = (app_font, 12, "bold"), width = 30, bd = 5)
        self.entry_username.place(x = 64, y = 265)
    ...
    def confirm(self):
        ...
        if username and answer:
            ...
            if data:
                if answer == data[0][0]:
                    self.delete()
                    self.controller.frames["User_New_Password"].username = username
                    self.controller.show_frame("User_New_Password")
                else:
                    self.delete()
                    messagebox.showerror(title = "Forgot password", message = "Username or answer invalid!")
            else:
                self.delete()
                messagebox.showerror(title = "Forgot password", message = "Username or answer invalid!")
        else:
