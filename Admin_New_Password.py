...
class Admin_New_Password(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.label_password = tk.Label(self, text = "Input new password (10 digits)", font = (app_font, 12))
            ...
            if password.isnumeric():
                if len(password) == 10:
                    if password == password_again:
                        code = "CALL CHANGE_ADMIN_PASSWORD('%s', '%s')"%(self.adminname, password)
                    ...
                    messagebox.showerror(title = "New password", message = "Password must have a length is 10!")
            else:
                messagebox.showerror(title = "New password", message = "Password must be a sequence of 10 digits!")
        else:
