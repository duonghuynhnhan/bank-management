...
class Admin_Edit_Personal_Information(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.textbox_address.insert("1.0", self.text_address)
    ...
    def update(self):
        ...
        self.label_fullname.place(x = 371-(len(self.text_fullname)*8.6), y = 160)
    ...
    def back(self):
        ...
        self.textbox_address.delete("1.0", "end-1c")
        self.textbox_address.insert("1.0", self.text_address.upper())
        self.controller.show_frame("Admin_Personal_Information")
        ...
        if email and address:
            if len(email) <= 100 and len(address) <= 100:
                if "@" in email and " " not in email:
                    code = """CALL CHANGE_ADMIN_PERSONAL('%s', '%s', '%s')"""%(self.adminname, email, address.upper())
                    self.controller.cursor.execute(code)
                    done = messagebox.showinfo(title = "Chang personal information", message = "You have change personal information successfully!")
                    if done:
                        self.controller.update("Admin_Personal_Information")
                        self.controller.update("Admin_Edit_Personal_Information")
                        self.controller.show_frame("Admin_Personal_Information")
                else:
