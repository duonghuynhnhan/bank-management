...
class Admin_Change_Answer(tk.Frame):
    def __init__(self, parent, controller):
        ...
        self.button_show_answer_again = tk.Button(self, text = "SHOW", font = (app_font, 8, "bold"), width = 5, border = "0", command = self.show_answer_again)
        self.button_show_answer_again.place(x = 325, y = 375)
        ...
        if oldanswer and newanswer and newansweragain:
            ...
            if oldanswer == db_answer:
                if newanswer == newansweragain:
                    code = "CALL CHANGE_ADMIN_ANSWER('%s', '%s')"%(self.adminname, newanswer)
                    self.controller.cursor.execute(code)
